...
class HeaderManager {
    ...
    setupHesabıTamamlaEvents() {
        ...
        if (saveAccountBtn) {
            saveAccountBtn.addEventListener('click', () => {
                ...
                console.log('💾 Tüm bilgiler kaydediliyor:', { tcValue, adresValue, ibanValue, ulkeValue, sehirValue, adresDetayValue });

                // Server'a kaydet
                this.saveAccountInfo(tcValue, adresValue, ibanValue, ulkeValue, sehirValue, adresDetayValue);
            });
        }
    }

    /**
     * Hesap bilgilerini server'a kaydet (Ödeme + Adres)
     */
    async saveAccountInfo(tcNo, adres, iban, ulke, sehir, adresDetay) {
        try {
            const token = localStorage.getItem('token');
            const user = localStorage.getItem('user');
    ...
    /**
     * Ödeme bilgilerini server'a kaydet (eski fonksiyon - artık saveAccountInfo tarafından çağrılıyor)
     */
