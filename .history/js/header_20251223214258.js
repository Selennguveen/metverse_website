...
class HeaderManager {
    constructor() {
        console.log('🔷 HeaderManager constructor çalıştı');
        this.loadHeader();
    }
     ...
     */
    async loadHeader() {
        console.log('📥 loadHeader() başladı');
        try {
            const response = await fetch('/includes/header.html');
            console.log('📡 Fetch response:', response.status, response.statusText);
            
            if (!response.ok) {
                console.error('⚠️ Header dosyası yüklenemedi:', response.status, response.statusText);
                return;
            }

            const headerHTML = await response.text();
            console.log('✅ Header HTML text yüklendi, boyut:', headerHTML.length);
            
            const headerContainer = document.getElementById('header-container');
            console.log('🔍 header-container element:', headerContainer);
            
            if (headerContainer) {
                headerContainer.innerHTML = headerHTML;
                console.log('✅ Header HTML DOM'a eklendi');
                
                // Header yüklendikten sonra logic'i kur
                setTimeout(async () => {
                    console.log('⏱️ setTimeout içinde - setupHeaderLogic() çağrılıyor');
                    this.setupHeaderLogic();
                    await this.updateHeaderView();
                    this.setupPremiumFeatureLinks();
                    console.log('✅ Header yüklendi ve updateHeaderView() çalıştı');
                }, 0);
            } else {
                console.error('❌ header-container element bulunamadı');
            }
        } catch (error) {
    ...
    /**
     * Premium özellik linklerini (Analizler) kur - paket kontrolü yap
     */
    setupPremiumFeatureLinks() {
        ...
        premiumLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                try {
                    const userData = JSON.parse(user);
                    const paket = userData.paket || 'ucretsiz';

                    if (paket !== 'premium') {
                        e.preventDefault();
                        this.showPremiumUpgradeModal(userData);
                        return;
                    }
                    // Premium ise normal şekilde linke git
                } catch (error) {
                    console.error('User paket kontrolü hatası:', error);
                }
            });
        });
    }
    ...
    /**
     * Premium Upgrade Modal'ını göster
     */
    showPremiumUpgradeModal(userData) {
        ...
        const modalHTML = `
            <div id="premiumUpgradeModal" class="modal-overlay">
                <div class="modal-container premium-upgrade-modal">
                    <button class="modal-close" onclick="document.getElementById('premiumUpgradeModal').remove()">×</button>
                    <div class="modal-content">
                        <h2>🚀 Premium Özellikler</h2>
                        <p class="premium-message">Analiz ve raporlama özelliklerine erişmek için hesabınızı premium yapmanız gerekiyor.</p>
                        
                        <div class="upgrade-info">
                            <h3>Premium Avantajları:</h3>
                            <ul>
                                <li>📊 Detaylı Analizler ve Raporlar</li>
                                <li>📈 İleri Seviye İstatistikler</li>
                                <li>💡 Özel İçgörüler ve Tavsiyeler</li>
                                <li>⚡ Hızlı Destek ve Danışmanlık</li>
                            </ul>
                        </div>

                        <div class="modal-actions">
                            <button class="btn-primary" onclick="window.location.href='/upgrade-plan'">
                                Hesabı Premium Yükseltin
                            </button>
                            <button class="btn-secondary" onclick="document.getElementById('premiumUpgradeModal').remove()">
                                Daha Sonra
                            </button>
                        </div>
                    </div>
