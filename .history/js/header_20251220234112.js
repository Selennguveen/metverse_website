...
class HeaderManager {
    ...
    async loadBaglantilarContent() {
        try {
            ...
            const userPlatform = influencerRecord.influencer_platform;
            const isConnected = connectedPlatforms.includes(userPlatform);
            
            // Bağlantı Durumu ve Buton
            html += `
                <div class="baglantı-button-container platform-connect-buttons">
            `;
            
            if (isConnected) {
                // Bağlı ise: Durum göster + Tekrar Bağla
                html += `
                    <div class="platform-status-display">
                        <span class="status-badge connected">✅ Bağlı</span>
                    </div>
                    <button class="btn-platform-baglantı tekrar-baglantı" onclick="window.headerManager.connectPlatform('${userPlatform}')">
                        🔗 Tekrar Bağla
                    </button>
                `;
            } else {
                // Bağlı değil ise: Platform spesifik butonlar
                html += `
                    <button class="btn-platform-baglantı instagram-baglantı" onclick="window.headerManager.connectPlatform('Instagram')">
                        📸 Instagram Bağla
                    </button>
                    <button class="btn-platform-baglantı youtube-baglantı" onclick="window.headerManager.connectPlatform('YouTube')">
                        ▶️ YouTube Bağla
                    </button>
                    <button class="btn-platform-baglantı tiktok-baglantı" onclick="window.headerManager.connectPlatform('TikTok')">
                        🎵 TikTok Bağla
                    </button>
                `;
            }
            ...
            if (!document.getElementById('baglantilar-modal-styles')) {
                ...
                style.textContent = `
                    ...
                    .tekrar-baglantı {
                        background: linear-gradient(135deg, rgb(76, 175, 80, 0.4), rgb(76, 175, 80, 0.3));
                        border-color: rgba(76, 175, 80, 0.6);
                        color: white;
                    }
                    
                    .tekrar-baglantı:hover {
                        background: linear-gradient(135deg, rgb(76, 175, 80, 0.6), rgb(76, 175, 80, 0.5));
    ...
    /**
     * Bağlı olan platformları al
     */
    getConnectedPlatforms() {
        try {
            const connectedStr = localStorage.getItem('connectedPlatforms');
            return connectedStr ? JSON.parse(connectedStr) : [];
        } catch (error) {
            console.error('❌ Bağlı platformlar okuma hatası:', error);
            return [];
        }
    }

    /**
     * Platform bağlantı durumunu kaydet
     */
    saveConnectedPlatform(platform) {
        try {
            const connected = this.getConnectedPlatforms();
            if (!connected.includes(platform)) {
                connected.push(platform);
                localStorage.setItem('connectedPlatforms', JSON.stringify(connected));
                console.log('✅ Bağlı platformlar kaydedildi:', connected);
            }
        } catch (error) {
            console.error('❌ Bağlı platformlar kayıt hatası:', error);
        }
    }
    ...
    connectPlatform(platform) {
        ...
        this.saveConnectedPlatform(platform);
        ...
        // Modal'ı kapat ve içeriği yenile
        setTimeout(() => {
            this.closeBaglantilarModal();
            this.loadBaglantilarContent();
            this.openBaglantilarModal();
        }, 500);
    }
