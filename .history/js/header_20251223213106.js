...
window.addEventListener('scroll', () => {
    ...
    /**
     * Premium Özellikleri Koru - Sadece premium paket'e sahip kullanıcılar erişebilsin
     */
    protectPremiumFeatures() {
        const user = localStorage.getItem('user');
        const premiumLinks = document.querySelectorAll('.premium-feature-link');

        if (!user || !premiumLinks.length) return;
        ...
        try {
            ...
            premiumLinks.forEach(link => {
                if (paket === 'ucretsiz') {
                    // Ücretsiz paket: Linki deaktif et
                    link.classList.add('premium-disabled');
                    ...
                    link.addEventListener('click', (e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        alert('🔒 Bu özellik sadece Premium paket için kullanılabilir. Upgrade olmak için Profil > Hesabı Tamamla kısmına gidin.');
                    });

                    console.log('🔒 Premium link deaktif edildi:', link.textContent);
                } else {
                    // Premium paket: Normal bırak
                    link.classList.remove('premium-disabled');
                    link.style.cursor = 'pointer';
                    link.style.opacity = '1';
                    console.log('✅ Premium link aktif:', link.textContent);
                }
            });
        } catch (error) {
            console.warn('⚠️ Premium kontrol hatası:', error);
        }
    }
}
