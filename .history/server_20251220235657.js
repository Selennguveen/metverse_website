...
// Hesap Bilgilerini Kaydet (Ödeme + Adres Bilgileri)
app.post('/api/save-account-info', async (req, res) => {
    const token = req.headers.authorization?.replace('Bearer ', '');
    const { influencerId, tcNo, adres, iban, ulke, sehir, adresDetay } = req.body;

    if (!token) {
        return res.status(401).json({ success: false, message: 'Token gerekli.' });
    }

    if (!influencerId || !tcNo || !adres || !iban || !ulke || !sehir || !adresDetay) {
        return res.status(400).json({ success: false, message: 'Tüm alanlar gerekli.' });
    }
    ...
    try {
        ...
        // Influencer'ın adres bilgilerini güncelle
