...
app.post('/api/kampanya-ekle', (req, res) => {
    try {
        ...
        // Gerekli alanları kontrol et
        if (!yeniKampanya.kampanya_adi || !yeniKampanya.influencer_id) {
            return res.status(400).json({ success: false, message: 'Gerekli alanlar eksik.' });
        }
        ...
        console.log('✅ Yeni kampanya eklendi:', yeniKampanya.kampanya_adi);
        res.json({ success: true, message: 'Kampanya başarıyla kaydedildi.', kampanya: yeniKampanya });
    } catch (error) {
        console.error('❌ Kampanya ekleme hatası:', error);
...
// Teklif Sil
app.delete('/api/teklif-sil/:id', (req, res) => {
    try {
        const teklifId = parseFloat(req.params.id);
        
        // Teklifleri oku
        const teklifler = readData(yapılanTekliflerFile);
        
        // Teklifi bul ve sil
        const yeniTeklifler = teklifler.filter(t => t.id !== teklifId);
        
        if (yeniTeklifler.length === teklifler.length) {
            return res.status(404).json({ success: false, message: 'Teklif bulunamadı.' });
        }
        
        // Dosyaya yaz
        writeData(yapılanTekliflerFile, yeniTeklifler);
        ...
        res.status(500).json({ success: false, message: 'Teklif silinirken hata oluştu.' });
    }
});
