...
function initializeDatabase() {
    ...
    if (!fs.existsSync(yapılanTekliflerFile)) {
        fs.writeFileSync(yapılanTekliflerFile, JSON.stringify([], null, 2));
    }
}
...
app.post('/api/register-influencer', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: '✅ İnfluencer kaydı başarılı.', token, user: { id: newInfluencer.id, ad, soyad, email, telefon, instagramHandle, takipciSayisi, niche, userType: 'influencer' } });
...
// Influencer Kayıt (Plan seçimi ile)
app.post('/api/kayit', async (req, res) => {
    const { tumKayit, ad, soyad, email, telefon, sifre, sifreTekrar, kosullar, paket } = req.body;
    
    if (tumKayit !== 'influencer') {
        return res.status(400).json({ success: false, message: 'Geçersiz kayıt türü.' });
    }

    // Doğrulama
    if (!ad || !soyad || !email || !telefon || !sifre || !sifreTekrar) {
        return res.status(400).json({ success: false, message: 'Tüm alanlar gerekli.' });
    }
    ...
    if (sifre.length < 6) {
        return res.status(400).json({ success: false, message: 'Şifre minimum 6 karakter olmalıdır.' });
    }
    ...
    try {
        ...
        const newInfluencer = {
            ...
            sifre: hashedPassword,
            telefon,
            kosullarKabul: kosullar ? 1 : 0,
            paket: paket || 'ucretsiz',
            platform: null,
            bio: null,
            takipci: null,
            takip_edilen: null,
        ...
        const token = jwt.sign({ id: newInfluencer.id, email }, JWT_SECRET, { expiresIn: '24h' });
        res.json({ success: true, message: '✅ İnfluencer kaydı başarılı.', token, user: { id: newInfluencer.id, ad, soyad, adSoyad: `${ad} ${soyad}`, email, telefon, paket: paket || 'ucretsiz', userType: 'influencer' } });
        console.log('✅ İnfluencer kaydı başarılı:', ad, soyad, email, '- Paket:', paket || 'ucretsiz');
    } catch (error) {
        res.status(500).json({ success: false, message: 'Kayıt sırasında hata oluştu.' });
    }
});
...
app.post('/api/register-marka', async (req, res) => {
    const { sirketAdi, markaAdi, email, sifre, sorumluIsim, sorumluTelefon, sorumluPozisyon, kosullarKabul, paket } = req.body;
    ...
    try {
        ...
        const newMarka = {
            ...
            sorumluIsim, 
            sorumluTelefon,
            sorumluPozisyon: sorumluPozisyon || '',
            kosullarKabul: kosullarKabul ? 1 : 0,
            paket: paket || 'ucretsiz',
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        ...
        res.json({ success: true, message: '✅ Marka kaydı başarılı.', token, user: { id: newMarka.id, sirketAdi, markaAdi, email, sorumluIsim, sorumluTelefon, paket: paket || 'ucretsiz', userType: 'marka' } });
        console.log('✅ Marka giriş başarılı:', markaAdi, email, '- Paket:', paket || 'ucretsiz');
    } catch (error) {
        res.status(500).json({ success: false, message: 'Kayıt sırasında hata oluştu.' });
    }
});
...
app.post('/api/influencer-login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: user.id, email }, JWT_SECRET, { expiresIn: '24h' });
        const userData = { id: user.id, ad: user.ad || '', soyad: user.soyad || '', adSoyad: (user.ad || '') + ' ' + (user.soyad || ''), email, telefon: user.telefon, instagramHandle: user.instagramHandle, paket: user.paket || 'ucretsiz', userType: 'influencer' };
        res.json({ success: true, message: '✅ Giriş başarılı.', token, user: userData, redirectUrl: '/influencer_anasayfa.html' });
...
app.post('/api/marka-login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: user.id, email }, JWT_SECRET, { expiresIn: '24h' });
        const userData = { id: user.id, sirketAdi: user.sirketAdi, markaAdi: user.markaAdi, email, sorumluIsim: user.sorumluIsim, paket: user.paket || 'ucretsiz', userType: 'marka' };
        res.json({ success: true, message: '✅ Giriş başarılı.', token, user: userData, redirectUrl: '/marka_anasayfa.html' });
...
app.get('/api/influencer/:id', (req, res) => {
    try {
        ...
        if (!influencer) {
            return res.status(404).json({ success: false, message: 'İnfluencer bulunamadı.' });
        }
        ...
        res.json({ 
            ...
            data: {
                id: influencer.id,
                paket: influencer.paket || 'ucretsiz'
            }
        });
    } catch (error) {
...
// Get Marka Data by ID
app.get('/api/marka/:id', (req, res) => {
    try {
        const { id } = req.params;
        const markalar = readData(markalarFile);
        const marka = markalar.find(m => m.id == id);
...
// Marka Premium'a Yükselt
app.post('/api/upgrade-marka-to-premium', async (req, res) => {
    const { markaId, markaAdi, email, kartNumarasi, kartAdi, kartAy, cvv } = req.body;

    try {
        console.log('💎 Marka Premium paketige yukseltiliyor:', { markaId, markaAdi, email });

        // Markalar.json'u oku
        const markalar = readData(markalarFile);

        // Marka'yı bul ve paket'ini güncelle
        const markaIndex = markalar.findIndex(m => m.id === markaId);

        if (markaIndex === -1) {
            console.warn('❌ Marka bulunamadı:', markaId);
        ...
        markalar[markaIndex].paket = 'premium';
        markalar[markaIndex].updatedAt = new Date().toISOString();

        // Dosyaya yaz
        writeData(markalarFile, markalar);

        console.log('✅ Marka Premium paketine yükseltildi:', markaId);

        res.json({ 
            success: true, 
            message: 'Hesabınız başarıyla Premium paketine yükseltildi.',
            paket: 'premium'
        });

    } catch (error) {
        console.error('❌ Marka yükseltme hatası:', error);
        res.status(500).json({ 
            success: false, 
...
// İnfluencer Premium'a Yükselt
app.post('/api/upgrade-influencer-to-premium', async (req, res) => {
    const { influencerId, adSoyad, email, kartNumarasi, kartAdi, kartAy, cvv } = req.body;

    try {
        console.log('💎 İnfluencer Premium paketige yukseltiliyor:', { influencerId, adSoyad, email });

        // İnfluencers.json'u oku
        const influencers = readData(influencersFile);

        // İnfluencer'ı bul ve paket'ini güncelle
        const infIndex = influencers.findIndex(i => i.id === influencerId);

        if (infIndex === -1) {
            console.warn('❌ İnfluencer bulunamadı:', influencerId);
            return res.status(404).json({ 
                success: false, 
                message: 'İnfluencer bulunamadı.' 
            });
        }

        // Paket bilgisini güncelle
        influencers[infIndex].paket = 'premium';
        influencers[infIndex].updatedAt = new Date().toISOString();

        // Dosyaya yaz
        writeData(influencersFile, influencers);

        console.log('✅ İnfluencer Premium paketine yukseltildi:', influencerId);
        ...
        console.error('❌ İnfluencer yükseltme hatası:', error);
        res.status(500).json({ 
            success: false, 
            message: 'Bir hata oluştu. Lütfen tekrar deneyin.' 
        });
...
app.post('/api/save-marka-verification-info', (req, res) => {
    const { markaId, markaAdi, adresUlke, adresSehir, acikAdres, vergino, kartNumarasi, kartAdı, kartAyAl, cvv, eposta } = req.body;

    console.log('📥 Doğrulama bilgileri POST isteği:', { markaId, markaAdi, adresUlke, adresSehir, acikAdres, vergino, eposta });

    // Validation
    if (!markaId || !markaAdi || !adresUlke || !adresSehir || !acikAdres || !vergino || !kartNumarasi || !kartAdı || !kartAyAl || !cvv || !eposta) {
        console.warn('⚠️ Validation hatası: Eksik alanlar');
        console.log('Alınan veriler:', { markaId, markaAdi, adresUlke, adresSehir, acikAdres, vergino, kartNumarasi, kartAdı, kartAyAl, cvv, eposta });
    ...
    try {
        ...
        // Markalar CSV'sine adres bilgileri ekle
        try {
            const markalarCsv = readData(markasCsvFile);
            const markaIndex = markalarCsv.findIndex(m => m.id === markaId);
            
            if (markaIndex !== -1) {
                markalarCsv[markaIndex].adres_Ulke = adresUlke;
                markalarCsv[markaIndex].adres_Sehir = adresSehir;
                markalarCsv[markaIndex].acik_Adres = acikAdres;
                markalarCsv[markaIndex].Vergi_No = vergino;
                markalarCsv[markaIndex].updatedAt = new Date().toISOString();
                
                writeData(markasCsvFile, markalarCsv);
                console.log('✅ Marka CSV bilgileri güncellendi:', markaAdi);
            }
        } catch (csvError) {
            console.warn('⚠️ Marka CSV güncellemesi başarısız:', csvError.message);
        }
...
// Teklif Ekle
app.post('/api/teklif-ekle', (req, res) => {
    try {
        const yeniTeklif = req.body;
        
        // Gerekli alanları kontrol et
        if (!yeniTeklif.markaId || !yeniTeklif.influencerId) {
            return res.status(400).json({ success: false, message: 'Gerekli alanlar eksik.' });
        }

        // Teklifleri oku
        const teklifler = readData(yapılanTekliflerFile);
        
        // Yeni teklifi ekle
        teklifler.push(yeniTeklif);
        
        // Dosyaya yaz
        writeData(yapılanTekliflerFile, teklifler);
        
        console.log('✅ Yeni teklif eklendi:', yeniTeklif.markaAdi, '-', yeniTeklif.influencerAdi, '-', yeniTeklif.kampanyaAdi);
        res.json({ success: true, message: 'Teklif başarıyla kaydedildi.', teklif: yeniTeklif });
    } catch (error) {
        console.error('❌ Teklif ekleme hatası:', error);
        res.status(500).json({ success: false, message: 'Teklif kaydedilirken hata oluştu.' });
    }
});

// Teklifi Kabul Et
app.post('/api/teklif-kabul', (req, res) => {
    try {
        const { offerId, kampanya } = req.body;
        ...
        // Teklifleri oku
        const teklifler = readData(yapılanTekliflerFile);
        const teklif = teklifler.find(t => t.id === offerId);

        // Kampanyayı teklif verilerine göre güncelle
        const updatedKampanya = {
            ...kampanya,
            kampanya_turu: teklif?.kampanya_turu || kampanya.kampanya_turu,
            platform: teklif?.platform || kampanya.platform
        };

        // Kampanyaları oku
        const kampanyalar = readData(path.join(__dirname, 'data', 'kampanyalar.json'));
        
        // Yeni kampanyayı ekle
        kampanyalar.push(updatedKampanya);
        
        // Kampanyaları yaz
        writeData(path.join(__dirname, 'data', 'kampanyalar.json'), kampanyalar);
        
        // Teklifleri güncelle - durum ve kullanılan teklifi işaretle
        const updatedTeklifler = teklifler.map(t => 
            t.id === offerId ? { ...t, durum: 'Kabul Edildi' } : t
        );
        writeData(yapılanTekliflerFile, updatedTeklifler);
        
        console.log('✅ Teklif kabul edildi. Kampanya eklendi:', updatedKampanya.kampanya_adi);
        res.json({ success: true, message: 'Teklif kabul edildi.', kampanya: updatedKampanya });
    } catch (error) {
        console.error('❌ Teklif kabul hatası:', error);
        res.status(500).json({ success: false, message: 'Teklif kabul edilirken hata oluştu.' });
    }
});

// Teklifi Reddet
app.post('/api/teklif-reddet', (req, res) => {
    try {
        ...
        // Teklifi sil
        const updatedTeklifler = teklifler.filter(t => t.id !== offerId);
        writeData(yapılanTekliflerFile, updatedTeklifler);
        ...
        res.status(500).json({ success: false, message: 'Teklif reddedilemedi.' });
    }
});
...
app.post('/api/save-platform', async (req, res) => {
    ...
    try {
        ...
        // Influencers CSV dosyasını oku
        const csvFile = path.join(__dirname, 'data', 'influencers_csv.json');
        const influencersCSV = readData(csvFile);

        // Influencer'ı ID'ye göre bul (CSV'de)
        const influencerCSVIndex = influencersCSV.findIndex(inf => inf.influencer_id === parseInt(influencerId));

        if (influencerCSVIndex === -1) {
            return res.status(404).json({ success: false, message: 'Influencer bulunamadı.' });
        }

        const influencerCSV = influencersCSV[influencerCSVIndex];
        let currentPlatforms = influencerCSV.influencer_platform || '';
        ...
        // Platform'u mevcut listeye ekle (CSV'ye)
        let newPlatformValue;
        if (currentPlatforms && currentPlatforms.trim()) {
            newPlatformValue = currentPlatforms + ', ' + platform;
        } else {
            newPlatformValue = platform;
        }
        influencersCSV[influencerCSVIndex].influencer_platform = newPlatformValue;

        // Influencers.json dosyasını da güncelle
        const mainInfluencersFile = path.join(__dirname, 'data', 'influencers.json');
        const mainInfluencers = readData(mainInfluencersFile);

        const mainInfluencerIndex = mainInfluencers.findIndex(inf => inf.id === parseInt(influencerId));

        if (mainInfluencerIndex !== -1) {
            // Ana dosyada platform alanını güncelle (null'dan gerçek değere)
            mainInfluencers[mainInfluencerIndex].platform = newPlatformValue;
            writeData(mainInfluencersFile, mainInfluencers);
            console.log('📝 Ana influencers.json dosyası güncellendi');
        }

        console.log('📝 Güncellenen platform bilgisi:', {
            id: influencerCSV.influencer_id,
            kullanici_adi: influencerCSV.influencer_kullanici_adi,
            platformlar: newPlatformValue
        });

        // CSV dosyasına yaz
        writeData(csvFile, influencersCSV);

        res.json({ 
            success: true, 
            message: `✅ ${platform} başarıyla bağlandı.`,
            data: {
                influencerId: influencerCSV.influencer_id,
                influencerName: influencerCSV.influencer_kullanici_adi,
                platform: platform,
                allPlatforms: newPlatformValue
            }
        });
    } catch (error) {
